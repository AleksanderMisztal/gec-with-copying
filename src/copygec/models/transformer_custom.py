...
def make_model(vocab_s, pad_idx, num_layers=1, d_model=512, d_ff=2048, h=8, dropout=0.1, device=None):
...
class EncoderDecoder(nn.Module):
  ...
  def encode(self, src):
    ...
    return self.encoder(emb)#, src_padding_mask)
  ...
  def decode(self, tgt, memory):
    ...
    y = self.decoder(tgt_emb, memory, tgt_subsequent_mask)
    y = self.generator(y)
    return y
...
class MultiHeadedAttention(nn.Module):
  ...
  def forward(self, q, k, v, subsequent_mask=None):
    ...
    x, self.attn = sdp_attention(q, k, v, mask, self.dropout)
    # (bs*h, Nt, E) -> (Nt, bs, h*E)
